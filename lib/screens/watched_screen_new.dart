...
// import 'package:go_router/go_router.dart'; // Removido
...
class WatchedScreen extends StatelessWidget {
  ...
  void _removeFromWatched(BuildContext context, WatchedMovie watchedMovie) {
    final app = context.read<AppState>();
    app.removeFromWatched(watchedMovie.movie.id);
    
    if (context.mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('${watchedMovie.movie.title} removida de vistas'),
          action: SnackBarAction(
            label: 'Deshacer',
            onPressed: () => app.addToWatched(watchedMovie.movie, watchedMovie.userRating),
          ),
        ),
      );
    }
